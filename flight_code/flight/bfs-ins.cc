...
void BfsInsRun(SensorData &ref, InsData * const ptr) {
  ...
  if (!ins_initialized_) {
    ...
    if ((imu_->new_data) && (mag_->new_data) && gnss_ready) {
      ...
      /* Initialize AHRS accelerometer filters with dedicated low cutoff to reject
       * motor vibration beat frequencies before the Mahony gravity correction */
      ahrs_ax_.Init(cfg_.ahrs_accel_cutoff_hz, FRAME_RATE_HZ, accel_mps2_[0]);
      ahrs_ay_.Init(cfg_.ahrs_accel_cutoff_hz, FRAME_RATE_HZ, accel_mps2_[1]);
      ahrs_az_.Init(cfg_.ahrs_accel_cutoff_hz, FRAME_RATE_HZ, accel_mps2_[2]);
