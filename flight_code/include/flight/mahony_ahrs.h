...
class MahonyAhrs {
 public:
  ...
  void Update(const Eigen::Vector3f &gyro_radps,
    ...
    /* Only apply accelerometer correction when magnitude is within ±10% of 1g.
     * Matches Betaflight's gate: rejects vibration-induced bias and maneuver
     * acceleration that would drive the filter to the wrong attitude. */
    static constexpr float G = 9.81f;
    float recipAccNorm = accel_mps2.squaredNorm();
    if (recipAccNorm > (0.9f * G) * (0.9f * G) &&
        recipAccNorm < (1.1f * G) * (1.1f * G)) {
