...
% Max yaw rate [radps]
% Aircraft.Control.yaw_rate_max = deg2rad(500);
Aircraft.Control.yaw_rate_max = deg2rad(300);
...
% Aircraft.Control.P_yaw_rate = 0.1;
% Aircraft.Control.I_yaw_rate = 0.3;
% Aircraft.Control.D_yaw_rate = 0.002;

% Yaw rate controller PID gains (old)
Aircraft.Control.P_yaw_rate = 0.05;
Aircraft.Control.I_yaw_rate = 0.2;
Aircraft.Control.D_yaw_rate = 0.001;

% Yaw rate D Controller LPFT cutoff frequency [Hz]
Aircraft.Control.D_yaw_FLTR_CTOFF = 10;
% Aircraft.Control.D_yaw_FLTR_CTOFF = 1; % Using a lower cutoff frequency to filter the raw gyro signal

%% Rate controller parameters
% Max att rate [radps]
Aircraft.Control.att_rate_max = deg2rad(300);

% Att rate controller PID gains
% Aircraft.Control.P_att_rate = 0.08;
% Aircraft.Control.I_att_rate = 0.05;
% Aircraft.Control.D_att_rate = 0.002;

% Att rate controller PID gains (old)
...
% Aircraft.Control.D_att_FLTR_CTOFF = 30;
Aircraft.Control.D_att_FLTR_CTOFF = 40;

% Att rate err LPFT cutoff frequency [Hz]
Aircraft.Control.rate_FLTR_CTOFF = 10;
...
% Max att angle [rad]
Aircraft.Control.att_angle_lim = deg2rad(60);  

% Att cmd controller gains
% Aircraft.Control.P_att_angle = 1;
Aircraft.Control.P_att_angle = 2; % Beta-flight value
Aircraft.Control.I_att_angle = 0;
Aircraft.Control.D_att_angle = 0.001;

% Att err LPFT cutoff frequency [Hz]
Aircraft.Control.att_FLTR_CTOFF = 5;
...
Aircraft.Control.est_hover_thr = 0.5;
...
MinOm_rps = 0;
MaxPWM_ms = 2000;
MinPWM_ms = 1098; % This is increased to the minimum PWM when armed
...
%% New parameters for Beta controller
att_angle_lim = 60;
P_roll_phi = 5; % change from 5 to 2.5
max_rate = 0.6;

Pterm_scale = 0.032029;
Iterm_scale = 0.244381;
Dterm_scale = 0.000529;
FFterm_scale = 0.013754;

P_roll_p = Pterm_scale*45;
I_roll_p = Iterm_scale*80;
D_roll_p = Dterm_scale*30;
FF_roll = FFterm_scale*120;

P_pitch_theta = 5;

P_pitch_q = Pterm_scale*47;
I_pitch_q = Iterm_scale*84;
D_pitch_q = Dterm_scale*34;
FF_pitch = FFterm_scale*125;

yaw_rate_lim = 0.55;
P_yaw_r = Pterm_scale*45;
I_yaw_r = Iterm_scale*80;
FF_yaw = FFterm_scale*120;
...
f_cut_pt1 = 15;
dT = 0.01;
CUT_OFF_CORRECTION_PT3 = 1/sqrt(2^(1/3)-1);
omega_PT1 = 2*pi*f_cut_pt1*dT;
omega_PT3 = 2*pi*f_cut*CUT_OFF_CORRECTION_PT3*dT;
PT3_k = omega_PT3/(omega_PT3+1);
PT1_k = 0.2857;
PT1_k_Iterm = omega_PT1/(omega_PT1+1);

PWM_min = 1000+980*0.1;
PWM_max = 2000;